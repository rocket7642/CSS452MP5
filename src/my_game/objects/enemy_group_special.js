...
class EnemyGroupSpecial extends EnemyGroup {
  constructor(spriteTexture, xPos, yPos) {
    ...
    this.box = new engine.BoundingBox(
      vec2.fromValues((this.minX + this.maxX) / 2, (this.minY + this.maxY) / 2),
      this.maxX - this.minX,
      this.maxY - this.minY
    );
  }
  ...
  update() {
    ...
    if (
      this.topWing.terminate ||
      this.bottomWing.terminate ||
      this.middleWing.terminate
    ) {
      this.terminate();
    }
  }
